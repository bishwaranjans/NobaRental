    ...
    TimeProvider timeProvider) : ComponentBase
{
    ...
    protected PickupFormModel Model { get; set; } = new() { RegistrationNumber = string.Empty, CustomerSsn = string.Empty };
    protected bool SelectedCategoryChargesKilometers =>
        Categories.FirstOrDefault(
            x => string.Equals(x.Code, Model.CategoryCode, StringComparison.OrdinalIgnoreCase))
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            ...
            if (categories.ResponseMessage.IsSuccessStatusCode
                && categories.GetContent() is { } list)
            {
        ...
        {
            snackbar.AddError(
                $"Error loading pickup data: {ApiExceptionHelper.GetErrorMessage(ex)}");
        }
    }
    ...
    protected async Task LoadAvailableCarsForStation(string stationCode)
    {
        ...
        {
            snackbar.AddError(
                $"Error loading available cars: {ApiExceptionHelper.GetErrorMessage(ex)}");
        }
    }
    ...
    protected void OnCarSelected(string regNumber)
    {
        ...
        if (car is not null)
        {
            Model.CategoryCode = car.CategoryCode;
    ...
    protected static string GetCategoryName(string code, string? name) => name ?? code;
    protected void Cancel() => MudDialog.Cancel();

    protected async Task Submit()
    {
        if (Form is not null) await Form.ValidateAsync();
        if (!IsValid || string.IsNullOrWhiteSpace(Model.RegistrationNumber) || string.IsNullOrWhiteSpace(Model.CategoryCode)) return;
        IsSubmitting = true;
        try
        {
            var now = timeProvider.GetLocalNow();
            ...
            var request = new RegisterPickupRequest(
                ...
                Model.CustomerSsn.Trim(),
                Model.CategoryCode,
                SelectedStationCode,
                pickupDateTime,
                Model.PickupMeterReadingKm);
            ...
            {
                snackbar.AddSuccess(
                    $"Pickup registered successfully at {SelectedStationCode}! Booking: #{content.BookingNumber}");
                MudDialog.Close(DialogResult.Ok(content));
            }
            else snackbar.AddError(ApiExceptionHelper.GetErrorMessage(response.StringContent, "Failed to register pickup."));
        }
        catch (Exception ex)
        {
